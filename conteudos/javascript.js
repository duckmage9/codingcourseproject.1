 ...
 * AUTO-PATCH: Corrige o motor v6.1 em tempo real sem alterar o arquivo original
 */

// Usamos uma variável própria para não misturar com o HTML
window.conteudosJS = window.conteudosJS || {};

// O SISTEMA DE PATCH: Fica vigiando até o V6 nascer e então injeta as correções
const applyPatch = setInterval(() => {
    
    // Verifica se as três peças chaves do V6 já existem na memória do navegador
    if (typeof db !== 'undefined' && typeof renderEncGrid === 'function' && typeof openModal === 'function') {
        
        clearInterval(applyPatch); // Encontrou o motor! Pode parar de procurar.

        // --- CORREÇÃO 1: Injeta os títulos na const 'db' para gerar os cards ---
        db.js.iniciante = [
        ...
        // Atualiza a tela imediatamente caso a aba ativa já seja a de JS
        if (typeof currentTech !== 'undefined' && currentTech === 'js') {
            renderEncGrid();
        }

        // --- CORREÇÃO 2: Hackeia e substitui a função openModal original ---
