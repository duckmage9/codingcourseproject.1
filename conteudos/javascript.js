/**
 * BANCO DE DADOS DE TEXTOS - JAVASCRIPT COMPLETO (V BETA 0.2)
 * Injeção dinâmica compatível com a arquitetura v6.1 sem alteração de arquivos locais
 */

// 1. Injeta os títulos de JavaScript diretamente no objeto 'db' do seu script principal
if (typeof db !== 'undefined' && db.js) {
    db.js.iniciante = [
...
// 2. Cria a variável global esperada pelo openModal() do seu sistema
window.conteudosHTML = window.conteudosHTML || {};

// 3. Adiciona os textos de JS diretamente no indexador de leitura
window.conteudosHTML["Variáveis e Constantes (let e const)"] = `
...
window.conteudosHTML["Tipos de Dados Essenciais"] = `
...
window.conteudosHTML["Operadores Matemáticos e Lógicos"] = `
...
window.conteudosHTML["Estruturas Condicionais (if, else, else if)"] = `
...
window.conteudosHTML["Estruturas de Repetição (for e while)"] = `
...
window.conteudosHTML["Introdução às Funções"] = `
...
window.conteudosHTML["Arrays Simples (Listas)"] = `
...
window.conteudosHTML["Objetos Básicos (Chave e Valor)"] = `
...
window.conteudosHTML["Manipulação Básica do DOM (getElementById)"] = `
...
window.conteudosHTML["Eventos de Teclado e Mouse (addEventListener)"] = `
